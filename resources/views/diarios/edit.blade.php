...
<div class="row">
    ...
    <div class="col-sm-6">
        <form action="{{ route('diario.update', ['id' => $turma->id, 'diario' => $diario]) }}" method="POST">
        @method('PATCH')
        @csrf
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('data') is-invalid @enderror" value="{{ $diario->data }} " id="data" name="data">
                            @error('data')
    ...
    <div class="card-body">
        <table class="table">
            ...
            <tbody>
                ...
                <input type="hidden" name="id_hora_aula[]" value="{{ $aula->id }}">
                <tr>
                    <td>
                        {{ $aula->alunos->nome }}
                    </td>
                    <td>
                        ...
                        <select class="form-select " name="licao[]" id="licao">
                            @forelse($aula->livros->licoes as $licao)
                             <option value="{{ $licao->id }}" {{ $aula->licao_id == $licao->id ? 'selected' : '' }}>{{ $licao->nome }}</option>
                             @empty
                             @endforelse
                        </select>
                ...
                @include( 'diarios.observacoes-modal-edit', ['id' => $aula->alunos->id, 'obs' => isset($aula->observacoes) ? $aula->observacoes : ''  ] )
                @empty
                @endforelse
            </tbody>
        </table>
